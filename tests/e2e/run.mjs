...
async function jeopardyScenario(browser, step) {
  ...
  try {
    ...
    step('Правила как в телепередаче: 2 раунда по 4 темы и финал (вопросы — с листа ведущего)')
    await host.locator('button[title="Настройки"]').click()
    await host.locator('.modal label.field', { hasText: 'Правила' }).locator('select').selectOption('tv')
    await sleep(300)
    await s.shot(host, 'host-settings')
    await host.keyboard.press('Escape')
    await host.locator('.shape label.field', { hasText: 'Раундов' }).locator('input').fill('2')
    await host.locator('.shape label.field', { hasText: 'Раундов' }).locator('input').press('Tab')
    await sleep(200)
    await s.shot(host, 'host-lobby-shape')
    ...
    step('Ведущий вписывает названия тем из своего листа')
    host.once('dialog', (d) => d.accept('География'))
    await host.locator('.board .theme').first().click()
    host.once('dialog', (d) => d.accept('Кино'))
    await host.locator('.board .theme').nth(1).click()
    ...
    await host.locator('.q-head', { hasText: 'География' }).waitFor()
    await screen.getByText('Слушайте вопрос').waitFor()
    await s.shot(screen, 'screen-question-reading')
    await tap(boris)
    ...
    await screen.locator('.right-by', { hasText: 'Аня' }).waitFor()
    await s.shot(screen, 'screen-reveal')
    await host.keyboard.press('Enter')
    await host.locator('.board').waitFor()

    step('Кот в мешке: ведущий отмечает спецвопрос из своего листа')
    await host.locator('.board .cell').nth(2 * 5 + 2).click()
    await host.getByRole('button', { name: /Кот в мешке/ }).click()
    await host.getByText('отдаёт его другому игроку').waitFor()
    await screen.locator('.special-title', { hasText: 'Кот в мешке' }).waitFor()
...
async function sportScenario(browser, step) {
  ...
  try {
    ...
    host.once('dialog', (d) => d.accept('Столицы'))
    await host.locator('.th-name').first().click()
    const anna = phones['Аня']
    await anna.getByText('Вы капитан — кто играет?').waitFor()
    await anna.locator('.cap .theme', { hasText: 'Столицы' }).waitFor()
    ...
    await annaTheme('Тема 2').locator('.member', { hasText: 'Аня' }).click()
    await annaTheme('Тема 3').locator('.member', { hasText: 'Боря' }).click()
    ...
    await anna.locator('.cap .theme', { hasText: 'Тема 1' }).locator('.member', { hasText: 'Аня' }).click()
    await anna.getByRole('button', { name: /Готово/ }).click()
    await phones['Вика'].getByRole('button', { name: /Готово/ }).click()
...
async function khamsaScenario(browser, step) {
  ...
  try {
    ...
    // Темы раунда ведущий вписывает со своего листа, пока капитаны выбирают игрока.
    for (const [i, name] of ['Космос', 'Живопись', 'Компьютеры', 'Архитектура', 'Мифология'].entries()) {
      host.once('dialog', (d) => d.accept(name))
      await host.locator('.name-chip').nth(i).click()
      await host.locator('.name-chip', { hasText: name }).waitFor()
    }
    ...
    await host.locator('.strike-theme', { hasText: 'Архитектура' }).locator('.strike-act').click()
